...
retry_locked:
	...
	//pr_warn("alloc_slab_meta(%u) returning meta %lx-%lx, data %lx-%lx\n", order, meta_range_start, meta_range_last, data_range_start, data_range_end);
		...
		struct kmem_cache_order_objects oo)
{
	...
	/* Get a struct slab, either unused or new. */
	spin_lock_irqsave(&s->freed_slabs_lock, flags);
	if (unlikely(list_empty(freed_slabs))) {
	...
	/* Avoid making UAF reads easily exploitable by repopulating with pages
	 * containing attacker-controller data - always zero pages.
	 */
	if (!is_slab_pinned(slab)) {
		gfp_flags |= __GFP_ZERO;
...
void kfree(const void *x)
{
	...
	if (likely(SLAB_DATA_BASE_ADDR <= addr && addr < SLAB_END_ADDR)) {
		...
		if (folio_test_slab(folio)) {
			pr_emerg("%s: kfree(%px)\n", __func__, x);
			dump_page(virt_to_page(x), "unexpected slab page in kfree()");
			BUG();
		}
