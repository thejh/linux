...
#ifdef CONFIG_SLAB_FREELIST_HARDENED
	decoded = (void *)(ptr.v ^ slab->slab_cache->random ^ swab(ptr_addr));
#else
	decoded = (void *)ptr.v;
#endif
...
static inline void *get_freepointer(void *object, struct slab *slab)
{
	unsigned long ptr_addr;
	freeptr_t p;

	object = kasan_reset_tag(object);
	ptr_addr = (unsigned long)object + slab->slab_cache->offset;
	p = *(freeptr_t *)(ptr_addr);
	return freelist_ptr_decode(p, ptr_addr, slab);
}
 ...
 */
__no_kmsan_checks
static inline freeptr_t get_freepointer_safe(void *object, struct slab *slab)
{
	unsigned long freepointer_addr = (unsigned long)object +
		slab->slab_cache->offset;
	freeptr_t p;
		       ...
		       struct slab *slab)
{
	...
	for (p = slab->freelist; p; p = get_freepointer(p, slab))
...
static void print_trailer(struct kmem_cache *s, struct slab *slab, u8 *p)
{
	...
	pr_err("Object 0x%p @offset=%tu fp=0x%p\n\n",
	       p, p - addr, get_freepointer(p, slab));
					...
					void *object, u8 val)
{
	...
	if (!check_valid_pointer(s, slab, get_freepointer(p, slab))) {
...
static int on_freelist(struct kmem_cache *s, struct slab *slab, void *search)
{
	...
	while (fp && nr <= slab->objects) {
		...
		object = fp;
		fp = get_freepointer(object, slab);
		nr++;
	}
					   ...
					   int *cnt)
{
	...
	*tail = NULL;
	...
	do {
		...
		if (object != old_tail)
			next = get_freepointer(object, virt_to_slab(object));
		...
		struct kmem_cache_node *n, struct slab *slab, int orig_size)
{
	...
	slab->freelist = get_freepointer(object, slab);
	slab->inuse++;
			    ...
			    void *freelist)
{
	...
	while (freelist_iter) {
		nextfree = get_freepointer(freelist_iter, slab);
...
next_object:
	...
	if (object != tail) {
		object = get_freepointer(object, slab);
		goto next_object;
	}
	checks_ok = true;
...
load_freelist:
	...
	c->freelist = get_freepointer(freelist, c->slab);
	c->tid = next_tid(c->tid);
	local_unlock_irqrestore(&s->cpu_slab->lock, flags);
	return freelist;
...
check_new_slab:
	...
	if (unlikely(!pfmemalloc_match(slab, gfpflags))) {
		 ...
		 */
		deactivate_slab(s, slab, get_freepointer(freelist, slab));
		return freelist;
	}
...
redo:
	...
	if (!USE_LOCKLESS_FAST_PATH() ||
		...
		void *next_object;
		freeptr_t next_encoded = get_freepointer_safe(object, slab);

		if (unlikely(READ_ONCE(c->tid) != tid))
			goto redo;

		next_object = freelist_ptr_decode(next_encoded,
			...
			size_t size, void **p, struct obj_cgroup *objcg)
{
	...
	for (i = 0; i < size; i++) {
		...
		c->freelist = get_freepointer(object, c->slab);
		p[i] = object;
		maybe_wipe_obj_freeptr(s, p[i]);
	}
	c->tid = next_tid(c->tid);
...
static void early_kmem_cache_node_alloc(int node)
{
	...
	slab->freelist = get_freepointer(n, slab);
	slab->inuse = 1;
